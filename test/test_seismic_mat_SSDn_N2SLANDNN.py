...
def main(opt):
    ...
    data_dir = '/home/shendi_mcj/datasets/seismic/marmousi/f35_s256_o128/'
    original = sio.loadmat(data_dir + 'RN/0_1550.mat')['data'].squeeze()  # 0_1550
    ...
    plot_noise_space_cor(noise_space_cor(correlated_noise), 200, (3.2, 3), data_range=[0, 1], cmap=plt.cm.jet,
                         cbar=True)
    ...
    # sio.savemat(('../output/marmousi_eps/gt1550.mat'), {'data': GT[:, :]})
    # sio.savemat(('../output/marmousi_eps/noisy1550.mat'), {'data': noisy[:, :]})
    data = {}
    ...
    # generate alpha
    with torch.no_grad():
        N2SUNet = model.networks['N2SUNet'](data['L'])
        LAN = model.networks['LAN'](data['L'])
        UNet = model.networks['UNet'](data['L'])
    alpha = generate_alpha(N2SUNet, lower=0.05, upper=0.2)
...
if __name__ == '__main__':
    ...
    #two_stage_seis_BSN_invDN four_stage_seis_invDN two_stage_seis_N2S_invDN  four_stage_seis_invDN_v2 three_stage_SASS_v2
    ...
    opt['resume_from'] = '../train/logs/N2Sbt_LAN_DN_MmsF35/model_iter_00010000.pth'
