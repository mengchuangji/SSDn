...
class SeisMatTrainDataset(BaseTrainDataset):
    ...
    def _get_img_paths(self, path):
        ...
        self.img_paths = self.img_paths[int(0 / 40 * len(self.img_paths)):int(40 / 40 * len(self.img_paths))]#mcj
...
class SeisNpyTrainDataset_noisy(BaseTrainDataset):
    def __init__(self, path, patch_size, pin_memory):
        super(SeisNpyTrainDataset_noisy, self).__init__(path, patch_size, pin_memory)
    ...
    def _get_img_paths(self, path):
        self.img_paths = []
        L_pattern = os.path.join(path, 'RN/*.npy')  # 改为匹配 .npy 文件
        L_paths = sorted(glob.glob(L_pattern))
        for L_path in L_paths:
            self.img_paths.append({'L': L_path})
        self.img_paths = self.img_paths[:]  # 可根据需要再切片
...
class SeisMatValidationDataset(BaseTrainDataset):
    ...
    def __len__(self):
        return self.n_data
class SeisNpyValidationDataset(BaseTrainDataset):
    def __init__(self, path, patch_size, pin_memory):
        super(SeisNpyValidationDataset, self).__init__(path, patch_size, pin_memory)
        self._get_img_paths(path)
        self.n_data = len(self.img_paths)
    ...
    def __getitem__(self, index):
        ...
        img_L, img_H = self.crop(img_L, img_H)
        img_L = np.float32(np.ascontiguousarray(img_L))
        img_H = np.float32(np.ascontiguousarray(img_H))
        return {'L': img_L, 'H': img_H}

    def _get_img_paths(self, path):
        self.img_paths = []
        L_pattern = os.path.join(path, 'RN/*.npy')  # 匹配 .npy 文件
        L_paths = sorted(glob.glob(L_pattern))
        for L_path in L_paths:
            self.img_paths.append({'L': L_path, 'H': L_path.replace('RN', 'CL')})
        self.img_paths = self.img_paths[:]  # 可根据需要切片
